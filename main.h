 ...
 * @envar:env var to be evaulated
 * @value: value of env var
 * @next:next note in the list
 *
...
int _strlen(char *s);
bool is interctive(void);
char *_getenv(char *command, env_t *env);
int _putchar(char c);
int print_env(char **environ, env_t *env);
void exit_check(har **cmd, env_t *env);
...
env_t *init_env(env_t *env, char *programe);
...
char **tekenize(char *line, env_t *env);
